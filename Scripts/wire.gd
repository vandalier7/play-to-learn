...
var requiredStartingDirection: Vector2 = Vector2.RIGHT
...
func addWaypoint(globalPos: Vector2) -> void:
	# you cant add waypoints unless you started by clicking on a Port
	...
	if not _canTurn(waypoints.back(), end):
		return
	var segDir: Vector2 = (end - waypoints.back()).normalized()
	if waypoints.size() == 1 and requiredStartingDirection != Vector2.ZERO:
		if segDir != requiredStartingDirection:
			return
...
func endLine(pos: Vector2, endingDirection: Vector2) -> bool:
	...
	targetPos = pos
	targetFacingDir = endingDirection
...
func _routeToTarget(targetPosition: Vector2) -> bool:
	...
	var segments: Array[Vector2] = _solveRoute(start, target, committedDir, targetFacingDir)
	...
	for i in range(1, segments.size()):
		...
		if currentDir != Vector2.ZERO and segDir == -currentDir:
			return false
		if i == 1 and waypoints.size() == 1 and requiredStartingDirection != Vector2.ZERO:
			if segDir != requiredStartingDirection:
				return false
		currentDir = segDir
...
func _tryOneTurn(start: Vector2, target: Vector2, fromDir: Vector2, arrivalDir: Vector2) -> Array[Vector2]:
	if arrivalDir.x != 0:
		var mid: Vector2 = Vector2(start.x, target.y)
		var seg1Dir: Vector2 = Vector2(0, sign(target.y - start.y))
		var seg2Dir: Vector2 = Vector2(sign(target.x - start.x), 0)
		#print(seg1Dir)
		if fromDir != Vector2.ZERO and seg1Dir == -fromDir:
			return []
		if -seg1Dir == committedDir:
			return []
		if sign(target.x - mid.x) == sign(arrivalDir.x):
			var pts: Array[Vector2] = [start, mid, target]
			if _segmentsValidMinTravel(pts):
				return pts
	if arrivalDir.y != 0:
		var mid: Vector2 = Vector2(target.x, start.y)
		var seg1Dir: Vector2 = Vector2(sign(target.x - start.x), 0)
		var seg2Dir: Vector2 = Vector2(0, sign(target.y - start.y))
		#print(seg1Dir)
		#print(committedDir)
		if fromDir != Vector2.ZERO and seg1Dir == -fromDir:
			return []
		if seg1Dir == committedDir:
			return []
		if sign(target.y - mid.y) == sign(arrivalDir.y):
			var pts: Array[Vector2] = [start, mid, target]
			if _segmentsValidMinTravel(pts):
				return pts
	return []
...
func _tryTwoTurns(start: Vector2, target: Vector2, fromDir: Vector2, arrivalDir: Vector2) -> Array[Vector2]:
	...
	var mid: Vector2
	if arrivalDir.x != 0:
		mid = Vector2(start.x, target.y)
		if sign(target.x - mid.x) != sign(arrivalDir.x):
			return []
	else:
		mid = Vector2(target.x, start.y)
		if sign(target.y - mid.y) != sign(arrivalDir.y):
			return []
	if mid == start or mid == target:
		return []
	var seg1Dir: Vector2 = (mid - start).normalized()
	var seg2Dir: Vector2 = (target - mid).normalized()
	if fromDir != Vector2.ZERO and seg1Dir == -fromDir:
		return []
	if seg2Dir == -seg1Dir:
		return []
...
func _tryThreeTurns(start: Vector2, target: Vector2, fromDir: Vector2, arrivalDir: Vector2) -> Array[Vector2]:
	...
	if arrivalDir.x != 0:
		...
		if sign(target.x - m2.x) != sign(arrivalDir.x):
			return []
	else:
		m2 = Vector2(target.x, start.y)
		m1 = Vector2((start.x + target.x) / 2.0, start.y)
		if sign(target.y - m2.y) != sign(arrivalDir.y):
			return []
...
func _rebuildPreview(preview: Vector2) -> void:
	...
	if waypoints.size() == 1 and requiredStartingDirection != Vector2.ZERO:
		if (end - a).normalized() != requiredStartingDirection:
			if requiredStartingDirection.x != 0:
